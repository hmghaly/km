...
def app(environ, start_response):
    ...
    if script_name=="index":
        ...
        #category_select_dropdown
            ...
            #page_content=str(soup)
            #page_content=page_content.replace(div_str,"")
            # root = html.parse(cur_fpath).getroot()
            # element = root.get_element_by_id("directory_col1")
            # test_content=html.tostring(element).decode("utf-8")
            # test_content_tags=list(re.findall('<([^<>]*?)>', test_content))
            # new_content=test_content_tags[0]+test_content_tags[-1]
            # page_content=page_content.replace(test_content,new_content)
            

            #page_content="<html><body><h1>Test Page</h1> %s </body></html>"%test_content
            #page_content=page_obj.update_replace(repl_dict1,"our new title","and a description as well")        
            # for k,v in rec_child_dict.items(): 
            #     test_content+=str(k)+"<br>"
        except Exception as e:
          error=str(e)
        ...
        # try:
        #     link_template"category?country=%s&domain=%s&cat=%s&start_i=_s_i_"%(country,domain,cat)
        #     link_template_copy=link_template.replace("_s_i_","2")
        #     pagination_content+='<a href="%s">02</a>'%link_template_copy
        # except Exception as e:
        #     error=str(e)
        #     trace=traceback.format_exc()
        #     pagination_content=error_msg="<h1>Test Page</h1> Error: %s <br> Trace: %s"%(error,trace)
        ...
        # try:
        #     #Now let's do the pagination
        #     link_template"category?country=%s&domain=%s&cat=%s&start_i=_s_i_"%(country,domain,cat)
        #     cur_page_i=int(start_i/n_results)
        #     n_pages=math.ceil(num_lines/n_results) #the actual number of result pages is the number of lines of the cat file/n_results per page
        #     last_page_i=n_results*(n_pages-1)
        #     pagination_content=""
        #     pagination_content+='<span class="current">01</span>'
        #     link_template_copy=link_template.replace("_s_i_","2")
        #     pagination_content+='<a href="%s">02</a>'%link_template_copy
        #     link_template_copy=link_template.replace("_s_i_","3")
        #     pagination_content+='<a href="%s">03</a>'%link_template_copy

        #     link_template_copy=link_template.replace("_s_i_",str(last_page_i))
        #     pagination_content+='<a href="%s">%s</a>'%(link_template_copy,n_pages)
        ...
        num_lines=get_file_n_lines(cat_fpath)
        m_lines=get_multiple_lines(start_i,n_results,cat_fpath)
        results_list=[]
        for line_item in m_lines:
            out=mydict.get(line_item)
            results_list.append(out)
